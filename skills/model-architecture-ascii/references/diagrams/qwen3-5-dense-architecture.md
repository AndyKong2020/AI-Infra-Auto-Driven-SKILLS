...
- **FFN is dense at every layer**: `mlp_only_layers=[]` and there is no MoE config — every one of the 64 layers carries a single SwiGLU FFN (`intermediate_size=17408`). This is the structural axis that separates this entry from [[qwen3-5-moe-architecture]] (which uses `moe-shared+routed`). The MoE sibling has 60 layers in the same hybrid linear+GQA cycle, so the comparison is per-layer-FFN: dense-FFN here vs MoE-FFN there; all other axes (hybrid attention, MTP, vision tower) match.
