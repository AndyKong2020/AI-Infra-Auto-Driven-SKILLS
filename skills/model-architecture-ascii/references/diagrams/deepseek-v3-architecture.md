...
- **DeepSeek V3.2-Exp is a separate model** (`model_type=deepseek_v32`, `architectures=DeepseekV32ForCausalLM`); see [[deepseek-v3-2-exp-architecture]]. V3.2 replaces V3's `mla` attention with `dsa` (MLA + lightning indexer); the MoE block, MLA Q/K/V dims, layer count, and `first_k_dense_replace=3` are unchanged from V3. Do **not** alias V3.2 onto this V3 file — they have different `attention_type` tags. V3.2 also exposes `num_nextn_predict_layers=1` in config but does not realize MTP in its bundled `inference/model.py` (see V3.2-Exp Notes).
