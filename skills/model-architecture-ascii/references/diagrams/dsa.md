...
- **Per-model deployment specializations (not part of the DSA pattern itself)**: DeepSeek V3.2-Exp's bundled `inference/model.py` adds a Hadamard transform `rotate_activation` to indexer Q and K before scoring, quantizes them to FP8 (`e4m3`), and stores the indexer K-cache at FP8 with a separate FP32 scale cache. GLM-5's `GlmMoeDsaIndexer` does **not** do any of these — its indexer runs in bf16/fp32 with no rotation. These are V3.2-Exp deployment specializations layered on top of the shared DSA structure; they live in the V3.2-Exp model file's Notes, not in this model-agnostic pattern.
