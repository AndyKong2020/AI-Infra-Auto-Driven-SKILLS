...
- **Uniform attention across all 62 layers**: all 62 layers run the same `MiniMaxM2Attention` class (standard softmax + GQA + `repeat_kv`). There is **no MiniMax-M1-style lightning-attention / softmax hybrid** here — `attention_type=gqa` is the correct closed-vocab tag, and no `hybrid-*` extension is required. The `attn_type_list = [1, 1, …, 1]` field in config (length 62) is **not actually read by the published `modeling_minimax_m2.py`** — no dispatch on it; attention backend is selected only via `_attn_implementation`. The `layernorm_linear_attention_beta=1.0` field is in the same vestigial category (legacy from earlier MiniMax architectures, never read by current modeling code).
